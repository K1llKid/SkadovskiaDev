...
function PANEL:DoLamar()
    ...
    local function doPostText()
        ...
        self.backsoon:SetText("Nous seront bientôt de retour !")
...
function PANEL:PaintOver()
    draw.DrawText(":( Connection perdue", "ixTitleFont", ScrW() / 2, 10, color_white, TEXT_ALIGN_CENTER)

    if self.ServerIsOff == nil then
        draw.DrawText("Vérification de l'état du serveur...", "ixBigFont", ScrW() / 2, 130, color_white, TEXT_ALIGN_CENTER)
        return
    end

    if self.ServerIsOff then
        draw.DrawText("Le serveur a été mis hors ligne. Essayez de vous reconnecter dans quelques minutes.", "ixBigFont", ScrW() / 2, 130, color_white, TEXT_ALIGN_CENTER)
    else
        draw.DrawText("Vous avez perdu la connexion au serveur. Essayez de vous reconnecter dans quelques minutes.", "ixBigFont", ScrW() / 2, 130, color_white, TEXT_ALIGN_CENTER)
        draw.DrawText("Vérifiez votre routeur ou votre connexion Internet.", "ixBigFont", ScrW() / 2, h + 160, color_white, TEXT_ALIGN_CENTER)
    end
end
